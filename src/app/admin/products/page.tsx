...
import { TYPE_TAGS_MULTI_AXIS, CATEGORY_FEATURE_TAGS } from "@/lib/tag-definitions";
...
export default function AdminProductsPage() {
  ...
  const toggleTag = (tag: string) => {
    ...
    // 種類タグの排他制御: 同じ軸のタグを外す
    const axes = TYPE_TAGS_MULTI_AXIS[editForm.category] || [];
    ...
    if (newTags.includes(tag)) {
      ...
      for (const axis of axes) {
        if (axis.tags.includes(tag)) {
          newTags = newTags.filter((t) => !axis.tags.includes(t));
          break;
        }
      }
      newTags.push(tag);
  ...
  // --- タグ表示用ヘルパー ---
  const getTypeTags = (category: string) => TYPE_TAGS_MULTI_AXIS[category] || [];
  const getFeatureTags = (category: string) => CATEGORY_FEATURE_TAGS[category] || [];

  return (
    <main className="max-w-[1080px] mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-1">デスクツアー 商品管理</h1>
                ...
                <div className="p-4 bg-blue-50">
                  ...
                  {getTypeTags(editForm.category).length > 0 && (
                    <div className="mb-3">
                      ...
                      {getTypeTags(editForm.category).map((axis) => (
                        <div key={axis.axis} className="mb-1.5">
                          <span className="text-xs text-gray-400 mr-2">{axis.axis}:</span>
                  ...
                  {/* 特徴タグ */}
                  {getFeatureTags(editForm.category).length > 0 && (
                    <div className="mb-3">
                      <label className="text-xs text-gray-500 mb-1 block">特徴タグ</label>
                      <div className="flex flex-wrap gap-1">
                        {getFeatureTags(editForm.category).map((tag) => (
                          <button
                            key={tag}
                            onClick={() => {
