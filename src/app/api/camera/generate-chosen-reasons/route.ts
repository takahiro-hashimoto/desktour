 ...
 * コメント10件以上の撮影機材商品に対して「選ばれている理由TOP3」を一括生成
 */
export async function POST() {
  try {
    // 1. 全商品のコメントを集計（product_mentions_camera から reason を取得）
    ...
    for (const product of targets) {
      ...
      try {
        ...
        if (reasons.length > 0) {
          ...
          if (updateError) {
            console.error(`[CameraChosenReasons] DB update failed for ${productName}:`, updateError);
            errors++;
          } else {
            processed++;
        ...
        console.error(`[CameraChosenReasons] Error for ${productName}:`, err);
        errors++;
      }
    }

    return NextResponse.json({
      message: `${processed}件の撮影機材商品に「選ばれている理由」を生成しました`,
      processed,
      errors,
    ...
    console.error("[CameraChosenReasons] Unexpected error:", error);
