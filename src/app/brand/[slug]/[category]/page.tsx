...
import { BRAND_TAGS, slugToBrand, slugToCategory, PRODUCT_CATEGORIES, TYPE_TAGS } from "@/lib/constants";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${brand}の${category}一覧`;
  const description = `${brand}の${category}がデスクツアー動画・記事で実際に使用されている様子を、使用者のコメント付きでまとめています。`;

  return {
    title,
    description,
    alternates: { canonical: `/brand/${params.slug}/${params.category}` },
    openGraph: { title, description, url: `/brand/${params.slug}/${params.category}` },
  };
}
      ...
      <PageHeaderSection
        ...
        description={
          <>
            ...
            で{brand}の{category}が実際に使用されている様子を、使用者のコメント付きでまとめています。デスク環境構築の参考にご活用ください。
          </>
        }
        breadcrumbCurrent={category}
        breadcrumbMiddle={{ label: brand, href: `/brand/${params.slug}` }}
      />
