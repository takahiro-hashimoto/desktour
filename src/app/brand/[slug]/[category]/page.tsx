...
import { BRAND_TAGS, slugToBrand, slugToCategory, PRODUCT_CATEGORIES, SUBCATEGORIES } from "@/lib/constants";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!BRAND_TAGS.includes(brand) || !PRODUCT_CATEGORIES.includes(category)) {
    return { title: "ページが見つかりません" };
  }

  return {
    title: `${brand}の${category}一覧 | デスクツアーDB`,
    description: `${brand}の${category}がデスクツアー動画・記事で実際に使用されている様子を、使用者のコメント付きでまとめています。`,
  };
}
