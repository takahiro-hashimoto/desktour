...
import type { CameraVideo, CameraProductMention, CameraInfluencer, CameraVideoWithProductCount, CameraArticleWithProductCount, CameraArticle } from "./types-camera";
import type {
  ProductWithStats,
  ProductComment,
  SearchParams,
...
export async function getCameraProductRanking(limit = 20, category?: string) {
  let query = supabase
    ...
    .select(`
      *,
      mention_count:product_mentions_camera!inner(count)
    `)
    .neq("product_mentions_camera.confidence", "low")
...
// カテゴリー内のサブカテゴリー一覧を取得（商品が存在するもののみ、言及数順）
...
// 商品検索（一覧ページ用）
export async function searchCameraProducts(params: SearchParams): Promise<{
  ...
  // 種類タグフィルタ（subcategoryカラムで絞り込み）
  if (typeTag) {
    query = query.eq("subcategory", typeTag);
  }

  // レンズタグフィルタ（lens_tagsカラムで絞り込み）
  if (lensTag) {
    query = query.contains("lens_tags", [lensTag]);
  }

  // ボディタグフィルタ（body_tagsカラムで絞り込み）
  if (bodyTag) {
  ...
  let products: ProductWithStats[] = (data || []).map((product) => {
    let mentions = product.product_mentions_camera || [];
...
export async function getCameraProductCountByCategory(): Promise<Record<string, number>> {
  const { data, error } = await supabase
    .from("products_camera")
    .select("id, category, product_mentions_camera!inner(count)")
    .neq("product_mentions_camera.confidence", "low")
    .not("asin", "is", null);
...
export async function getCameraProductDetailByAsin(asin: string): Promise<ProductDetail | null> {
  ...
  return getCameraProductDetailCommon(product);
}
...
// 商品詳細の共通処理
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function getCameraProductDetailCommon(product: any): Promise<ProductDetail | null> {
  const productId = product.id;

  const mentions = product.product_mentions_camera || [];

  // コメント（全件）にサムネイル情報を含める
  const videoIds = mentions.filter((m: CameraProductMention) => m.video_id).map((m: CameraProductMention) => m.video_id);
  const articleIds = mentions.filter((m: CameraProductMention) => m.article_id).map((m: CameraProductMention) => m.article_id);

  // 動画情報を一括取得
  const { data: commentVideos } = videoIds.length > 0
    ? await supabase.from("videos_camera")
  ...
  const { data: commentArticles } = articleIds.length > 0
    ? await supabase.from("articles_camera")
  ...
  // コメント生成
  const allComments: ProductComment[] = mentions.map((m: CameraProductMention) => {
    if (m.video_id) {
  ...
  const coUsedProducts = await getCameraCoOccurrenceProducts(productId, 10, product.category);

  // カテゴリ内順位を計算
  const { categoryRank, totalInCategory } = await getCameraCategoryRank(productId, product.category);

  // 職業別統計を取得
  const occupationBreakdown = await getCameraOccupationBreakdownForProduct(mentions);

  // デスクセットアップ統計を取得
  const deskSetupStats = await getCameraSetupStatsForProduct(mentions);
...
async function getCameraCategoryRank(productId: string, category: string): Promise<{ categoryRank: number; totalInCategory: number }> {
  // 同じカテゴリの全商品を言及数でソートして取得
  const { data: products } = await supabase
    .from("products_camera")
    .select(`
      id,
      product_mentions_camera!inner (count)
    `)
    .eq("category", category)
    .neq("product_mentions_camera.confidence", "low");
  ...
  const sorted = products.map((p: any) => ({
    id: p.id,
    count: Array.isArray(p.product_mentions_camera) ? p.product_mentions_camera.length : (p.product_mentions_camera?.[0]?.count || 0),
...
// 商品の言及から職業別統計を取得
async function getCameraOccupationBreakdownForProduct(mentions: CameraProductMention[]): Promise<OccupationStat[]> {
    ...
    if (channelIds.length > 0) {
      ...
      for (const inf of influencers || []) {
        ...
        if (inf.occupation_tags && inf.occupation_tags.length > 0) {
          const primaryTag = selectCameraPrimaryOccupation(inf.occupation_tags);
          if (primaryTag) {
    ...
    for (const article of articles || []) {
      ...
      if (matchedInf && !countedInfluencerIds.has(matchedInf.id)) {
        if (matchedInf.occupation_tags && matchedInf.occupation_tags.length > 0) {
          const primaryTag = selectCameraPrimaryOccupation(matchedInf.occupation_tags);
          if (primaryTag) {
            tagCounts.set(primaryTag, (tagCounts.get(primaryTag) || 0) + 1);
...
async function getCameraSetupStatsForProduct(mentions: CameraProductMention[]): Promise<DeskSetupStat[]> {
  const videoIds = mentions.filter((m) => m.video_id).map((m) => m.video_id);
  const articleIds = mentions.filter((m) => m.article_id).map((m) => m.article_id);
  ...
  const { data: videos } = videoIds.length > 0
    ? await supabase.from("videos_camera").select("video_id, tags").in("video_id", videoIds)
    : { data: [] };

  // 記事からタグを取得
  const { data: articles } = articleIds.length > 0
    ? await supabase.from("articles_camera").select("url, tags").in("url", articleIds)
    : { data: [] };
...
export async function getCameraCoOccurrenceProducts(
  ...
  const compatibleCategories = currentProductCategory
    ? getCameraCompatibleCategories(currentProductCategory)
    : [];
...
export async function getCameraSiteStats(): Promise<SiteStats> {
  const [
    ...
    supabase.from("products_camera").select("*", { count: "exact", head: true }),
    supabase.from("product_mentions_camera").select("*", { count: "exact", head: true }),
    supabase.from("videos_camera").select("*", { count: "exact", head: true }),
    supabase.from("articles_camera").select("*", { count: "exact", head: true }),
    supabase.from("influencers_camera").select("*", { count: "exact", head: true }),
  ]);
...
export async function getCameraTopProductImages(limit = 24): Promise<string[]> {
  // product_mentions_cameraから商品ごとの言及数を集計（confidence lowを除外）
  const { data: mentions, error: mentionError } = await supabase
    .from("product_mentions_camera")
    .select("product_id")
    .neq("confidence", "low");
...
export async function getCameraTopProductByCategory(): Promise<
...
> {
  ...
  const [{ data: mentions, error: mentionError }, { data, error }] = await Promise.all([
    supabase.from("product_mentions_camera").select("product_id").neq("confidence", "low"),
    supabase.from("products_camera").select("id, name, category, amazon_image_url")
      .not("amazon_image_url", "is", null)
      .not("amazon_image_url", "eq", ""),
  ]);
...
export async function getCameraOccupationTagCounts(): Promise<Record<string, number>> {
  ...
  const [
    ...
    supabase.from("influencers_camera").select("channel_id, author_id, occupation_tags").not("occupation_tags", "is", null),
    supabase.from("videos_camera").select("video_id, channel_id"),
    supabase.from("articles_camera").select("url, author_url"),
  ]);
...
export async function getCameraSetupTagCounts(): Promise<Record<string, number>> {
  const [{ data: videos }, { data: articles }] = await Promise.all([
    supabase.from("videos_camera").select("tags").not("tags", "is", null),
    supabase.from("articles_camera").select("tags").not("tags", "is", null),
  ]);
...
export async function getCameraSourceDetail(
  ...
  if (sourceType === "video") {
    ...
    // product_mentions_camera.video_idにはYouTube動画ID(video.video_id)が入っている
    ...
    const productIds = (mentions || [])
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .filter((m: any) => m.products_camera)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const products: SourceProduct[] = (mentions || [])
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .map((m: any) => ({
        id: m.products_camera.id,
        slug: m.products_camera.slug,
    ...
    // influencers_camera テーブルから occupation_tags を取得
    ...
    // product_mentions_camera.article_idには記事のURL(article.url)が入っている
    const { data: mentions } = await supabase
      .from("product_mentions_camera")
      .select(`
        reason,
...
export async function getCameraVideos(params: {
  ...
  const videos = (data || []) as CameraVideo[];
  ...
  if (videos.length > 0) {
    ...
    // 動画に商品数を付与
    const videosWithCount: CameraVideoWithProductCount[] = videos.map((v) => ({
      ...v,
  ...
  return { videos: videos as CameraVideoWithProductCount[], total: count || 0 };
}
...
export async function getCameraArticles(params: {
  ...
  const articles = (data || []) as CameraArticle[];

  // 各記事の商品数を取得（confidence lowを除外）
  // product_mentions_camera.article_idには記事のURLが入っている
  if (articles.length > 0) {
    ...
    if (articleUrls.length > 0) {
      ...
      const articlesWithCount: CameraArticleWithProductCount[] = articles.map((a) => ({
        ...a,
  ...
  return { articles: articles as CameraArticleWithProductCount[], total: count || 0 };
}
...
export async function getCameraTopBrandsByProductCount(
  limit: number = 8
